...
### AR経由でディスパッチされる専門Agent（16名）

| Agent | 略称 | Prompt | 役割 |
|-------|------|--------|------|
| Requirements Analyst | REQ | requirements-analyst.md | 要件定義・PRD・受け入れ基準（Phase 0） |
...
mkdir -p docs/{requirements,architecture,design,design/wireframes,database,adr,api,operations}
```
...
```
Phase 0: Requirements（最上流）
  CEO: 人間に要件ヒアリング（CEOが唯一の窓口）→ 生データを .agent-team/prompts/ に保存
  CEO → AR → REQ: PRD・ユーザーストーリー・受け入れ基準・スコープ・NFR素案を構造化
  CEO → ★★★ Gate 0: 要件確認 ★★★
         REQの Open Questions（設計ブロッカー）を人間に確認
         ※ 要件は人間承認が本質のためEvaluatorは新設せず、CEOが人間に確認する軽量ゲート
         要件確定 → Phase 1 を解禁（ARCH/TL は docs/requirements/ を入力に設計する）

Phase 1: Architecture & Tech Design
  CEO → AR → ARCH: システム構造設計（レイヤー、モジュール、データフロー / REQの要件・NFR素案を基に）
...
Phase 1.7: Knowledge Sync（UIUX+DBA+SEC設計レビュー+TEST観点レビュー完了後）
  ...
  ※ INFRA/CICD/SREはPhase 1承認後にAR経由で開始可能（デザイン承認を待たない）
    - SRE: SLO/SLI定義・監視設計・ロールバック戦略を早期に固める（INFRA監視基盤・CICDデプロイゲートと連携）
...
Phase 3: Quality（実装完了後、品質レビューループ）
  ⚠️ REV/SEC/TEST の全てが合格 + QAの受け入れ判定APPROVEまで Phase 4 に進まない
  ...
  Step 2.5: CEO → AR → QA: REV/SEC/TEST結果と受け入れ基準を束ねて品質統括判定
    - QA: APPROVE（Quality Gate通過を具申）or CHANGES_REQUESTED（未充足項目と担当Agentを明示）
    - QAは個別判定では見えない品質リスク（受け入れ基準の未充足・テスト観点の欠落）を横断的に検出する
    - ⚠️ 品質の最終責任はCEOに残る。QAは判断を集約してCEOに具申する役
  Step 3: いずれかが不合格（REV/SEC/TEST いずれか or QA CHANGES_REQUESTED）の場合 → 品質修正ループ:
...
Phase 4-5: Integration → Release
  ...
  CEO → AR → SRE: Runbook整備・本番監視の最終確認・リリース判定の信頼性チェック（SLO達成・ロールバック手順の確認）
```

**⚠️ 絶対ルール:**
1. **CEOは専門Agentに直接ディスパッチしない — AR経由でディスパッチする**（KM・CG・ARCH-EVAL・DESIGN-EVALは例外）
2. **Gate 0（REQの要件確定）なしに ARCH/TL を開始しない**
3. **Gate 1（ARCH-EVAL APPROVE）なしに UIUX / DBA を開始しない**
4. **Gate 2（DESIGN-EVAL APPROVE）なしに PM を開始しない**
5. **PM の WBS 完了なしに FE/BE を開始しない**
6. **DBA のスキーマ設計完了なしに BE を開始しない**
7. **実装Agent（FE/BE）ディスパッチ前に KM で Agent別コンテキストを最新化する**
8. **EvaluatorがREJECTした場合、修正指示を対象Agentに伝え、修正後に再評価を必ず実施する。APPROVEまでループする**
...
- ヒアリング結果（生データ）を `.agent-team/prompts/` に保存する

### 1.5. AR経由でREQに要件構造化をディスパッチ（Phase 0 → Gate 0）

# CEO → AR: REQに要件定義をディスパッチ
Agentツールでディスパッチ:
- subagent_type: "agent-router"
- prompt: ディスパッチ指示書（対象Agent: REQ）の内容

ARへのディスパッチ指示書に含める情報:
- ヒアリング生データ（`.agent-team/prompts/`）
- REQに要求する出力: `docs/requirements/`（PRD・ユーザーストーリー・受け入れ基準・スコープ・NFR素案・Open Questions）

**★★★ Gate 0: 要件確認 ★★★**
- REQの `open-questions.md` のうち**設計をブロックする未決事項**を人間に確認する
- 要件は人間承認が本質のためEvaluatorは使わず、CEOが人間に確認する軽量ゲート
- **⚠️ 要件が確定するまで ARCH/TL を開始しない**
...
| ゲート | タイミング | 内容 | 評価者 |
|--------|-----------|------|--------|
| **Requirements Gate (Gate 0)** | **Phase 0完了** | **REQの要件・受け入れ基準・Open Questionsの確認** | **CEOが人間に確認** |
...
| **Quality Gate** | **Phase 3完了** | **REV APPROVE + SEC PASS + TEST全PASS + QA受け入れ判定APPROVE** | **★QA統括 + CEO（APPROVEまでループ）★** |

...
Human → CEO Agent (唯一の窓口)
          ...
          │     ├── SRE / Reliability (SRE)   # 運用・監視・SLO・インシデント
...
接続関係:
  ...
  CG → AR, FE, BE, INFRA, CICD, SRE, TEST, QA, REV, SEC, DOC: コンテキスト提供
...
│   ├── operations/                  # SRE + DOC 成果物（ファイル単位で所有分離）
│   │   ├── slo.md                   # SRE ★NEW
│   │   ├── monitoring-design.md     # SRE ★NEW
│   │   ├── runbook.md               # SRE ★NEW
│   │   ├── rollback-strategy.md     # SRE ★NEW
│   │   ├── capacity-plan.md         # SRE ★NEW
│   │   ├── postmortem-template.md   # SRE ★NEW
│   │   ├── local-setup.md           # DOC
│   │   ├── deploy-guide.md          # DOC
│   │   └── troubleshooting.md       # DOC
│   └── design-summary.md            # DOC成果物
├── infrastructure/
│   └── observability/               # SRE成果物（監視設定コード）★NEW
└── [frontend/ backend/ ...]
```
...
| # | Phase | 主要Agent | 完了条件 |
|---|-------|----------|---------|
| **0** | **Requirements ★NEW** | **REQ** | **PRD・受け入れ基準・スコープ・NFR素案完了 + ★Gate 0: CEOが人間に要件確認★** |
...
| 3 | Quality (Review Loop) | CG(analysis) → REV, SEC(実装レビュー), TEST(テスト実装) → QA(品質統括判定) | **REV APPROVE + SEC PASS + TEST全PASS + QA受け入れ判定APPROVE（不合格時はFE/BE修正→再レビューをループ）** |
| 4 | Integration | FE, BE, TEST, QA, SEC | 結合テスト通過 + QA受け入れ判定 |
| 5 | Release | TEST, SEC, CICD, SRE, DOC | 本番デプロイ + ドキュメント完備 + **SRE: Runbook・監視・SLO・ロールバック手順の確認** |

**フロー:**
```
Phase 0: CEO → 人間に要件ヒアリング → CEO → AR → REQ（要件構造化）→ ★Gate 0: CEOが人間に要件確認★
Phase 1: CEO → AR → ARCH → TL → ★Gate 1: ARCH-EVAL評価（APPROVEまでループ）★
  ...
  → Phase 3: CEO → AR → REV + SEC(実装レビュー) + TEST(テスト実装) → QA(品質統括判定: REV/SEC/TEST結果+受け入れ基準を束ねて受け入れ可否を具申)（全合格までループ: 不合格 → KM→CG → AR → FE/BE修正 → 再レビュー）
...
- **Gate 0（REQの要件確定）なしに、ARCH / TL を開始しない。**（要件はCEOが人間に確認する軽量ゲート）
...
- **Phase 3: REV/SEC/TESTのいずれかが不合格、またはQAの受け入れ判定がCHANGES_REQUESTEDの場合、FE/BE/TESTに修正を指示し、不合格だったAgentのみ再実行。全合格 + QA APPROVEまでループする（上限3回。3回で解消しない場合は人間にエスカレーション）。QAはREV/SEC/TESTを束ね受け入れ基準充足を統括判定するが、品質の最終責任はCEOに残る。**
- **PMのWBS完了なしに、FE/BEの実装を開始しない。**
- **DBAのスキーマ設計完了なしに、BEの実装を開始しない。**
- **実装Agent（FE/BE）ディスパッチ前にKMでAgent別コンテキストを最新化する。**
- INFRA/CICD/SREはPhase 1承認後にAR経由で開始可能（デザイン/PM完了を待たない）。SREはSLO/監視設計・ロールバック戦略を早期に固める。
...
| **SRE** | **docs/operations/ (slo.md, monitoring-design.md, runbook.md, rollback-strategy.md, capacity-plan.md, postmortem-template.md), infrastructure/observability/** |
| SEC | .agent-team/reviews/ (SEC分) |
| REV | .agent-team/reviews/ (REV分) |
| TEST | tests/ |
| **QA** | **docs/quality/, .agent-team/reviews/ (QA分)** |
...
| **DOC** | **README.md, CHANGELOG.md, docs/operations/ (local-setup.md, deploy-guide.md, troubleshooting.md), docs/design-summary.md, docs/glossary.md** |
